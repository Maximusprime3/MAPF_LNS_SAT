...
bool verify_path_consistency(
    ...
    for (size_t t = 0; t < path.size(); ++t) {
        ...
        if (pos.first < 0 || pos.first >= static_cast<int>(map.size()) ||
            ...
            std::cout << "[VerificationHelpers] ERROR: Path is out of bounds at timestep " << t << std::endl;
            return false;
        }
        // Check walkability
        if (map[pos.first][pos.second] != '.' && map[pos.first][pos.second] != 'G') {
            ...
            std::cout << "[VerificationHelpers] ERROR: Path is not walkable at timestep " << t << std::endl;
            return false;
        }
        ...
        if (!(same_row && same_col) && !neighbor_vertical && !neighbor_horizontal) {
            std::cout << "[VerificationHelpers] ERROR: Path is not consistent at timestep " << t
...
bool verify_solution_consistency(
    ...
    for (const auto& [agent_id, path] : agent_paths) {
        ...
        if (path.empty()) {
            std::cout << "[VerificationHelpers] ERROR: Agent " << agent_id << " has an empty path" << std::endl;
            return false;
        }
        if (path.front() != starts[agent_id]) {
            std::cout << "[VerificationHelpers] ERROR: Agent " << agent_id << " does not start at the start position" << std::endl;
            return false;
        }
        if (path.back() != goals[agent_id]) {
            std::cout << "[VerificationHelpers] ERROR: Agent " << agent_id << " does not end at the goal position" << std::endl;
            return false;
        }
        if (!verify_path_consistency(path, map)) {
            std::cout << "[VerificationHelpers] ERROR: Agent " << agent_id << " path is not consistent" << std::endl;
            return false;
        }
    }
    return true;

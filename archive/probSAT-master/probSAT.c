...
FILE *fp;
char *fileName;
char* initAssignmentsFile = NULL; // Path to the initial assignments of variables file, if provided
/*---------*/
...
void readInitialAssignments() {
    ...
    while (fscanf(fp, "%d %d", &var, &val) == 2) {
        ...
        // Print each assignment to verify correctness
        //printf("Assigning variable %d to value %d\n", var, (var > 0 ? atom[var] : !atom[-var]));
    }
    fclose(fp);
}
...
// Function to start the CPU timer
...
// Function to calculate the elapsed CPU time
...
// Function to start wall clock timer
...
// Function to calculate the elapsed wall clock time
...
int main(int argc, char *argv[]) {
	...
	if (maxTries > 1) {
		...
		printf("c %-30s: %-8.3fsec\n", "Mean solver time per try", totalSolverTime / (double) try);
		printf("c %-30s: %-8.3fsec\n", "Mean solver (s) time per try", totalSolverTime_s / (double) try);
	}
	return 0;
}

...
#include "lnssat/batch/simple_json.h"
...
fs::path check_executable(const char* program) {
    // Resolve the running batch binary, independently of the caller's working
    // directory. Canonicalization also follows symlinks to the build directory.
    fs::path runner;
#ifdef __linux__
    std::error_code ec;
    runner = fs::read_symlink("/proc/self/exe", ec);
    if (ec) runner.clear();
#endif
    if (runner.empty()) {
        fs::path invocation(program);
        if (invocation.has_parent_path()) {
            runner = fs::canonical(invocation);
        } else {
            const char* path_env = std::getenv("PATH");
            std::stringstream paths(path_env ? path_env : "");
            std::string directory;
            while (std::getline(paths, directory, ':')) {
                fs::path candidate = fs::path(directory.empty() ? "." : directory) / invocation;
                if (fs::is_regular_file(candidate) && ::access(candidate.c_str(), X_OK) == 0) {
                    runner = fs::canonical(candidate);
                    break;
                }
            }
        }
    ...
    if (!runner.empty()) {
        const fs::path solver = fs::canonical(runner).parent_path() / "lns-sat";
        if (fs::is_regular_file(solver) && ::access(solver.c_str(), X_OK) == 0) {
            return solver;
        }
    }
    throw std::runtime_error(
        "Required solver executable 'lns-sat' not found or not executable beside the batch runner. "
        "Build both with: cmake --build build (or make all)");
}
...
std::vector<std::string> build_command(const fs::path& exe, const Options& opts,
    ...
    // lns-sat accepts the neighborhood variant after the optional seed.
...
int execute_single_run(Options opts, const std::optional<std::string>& label,
    ...
    fs::path exe_path;
    try {
        exe_path = check_executable(program);
...
int main(int argc, char** argv) {
    ...
    if (opts.config_path) {
        ...
        try {
            ...
            for (std::size_t i = 0; i < runs.size(); ++i) {
                ...
                std::cout << "\n=== Running " << label << " ===\n";
                exit_code = execute_single_run(run.options, run.label, argv[0]);
    ...
    return execute_single_run(opts, std::nullopt, argv[0]);
}
